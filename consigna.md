...
- `spoticfy.sql`: Contiene el archivo SQL para importar la base de datos.
- `consigna.md`: El archivo que estás leyendo.
- `.REST`: Contiene los requests de la API para poder probarla con la extensión de VSCode REST Client. También pueden usar Postman o cualquier otra herramienta que te resulte cómoda.
...
- Crear la base de datos `spoticfy` en `neon`, conectarse desde `pgAdmin` y correr el script `spoticfy.sql` que se encuentra en la raíz del proyecto.
...
La fecha límite de entrega es el **viernes 4 de julio a las 23:59hs**. Una vez realizado algún commit en el repositorio, se considerará que la entrega está realizada. Se tomará el último commit realizado antes de la fecha y hora límite.
